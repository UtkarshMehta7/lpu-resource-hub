...
| `POST /api/v1/admin/users` | `user:update_role` (admin) | **The override.** The one endpoint where a role may be named. For what the hierarchy cannot serve — a department with no coordinator yet, a correction, a second admin. Audited as `user.created_by_admin`, never as an ordinary appointment. |
| `POST /api/v1/admin/users/{id}/admin-promotion` | `user:update_role` (admin) | Opens a two-person challenge: a six-digit code goes to the **target's** notification inbox, never the response. Ten-minute expiry, single use, five attempts. A new request supersedes the old one. |
| `POST /api/v1/admin/users/{id}/admin-promotion/confirm` | `user:update_role` (admin) | `{code}` — the digits the target read back. On success the target becomes an admin, audited as `user.promoted_to_admin` with the challenge id. `403` wrong code, `409` expired, `429` out of attempts. |
| `POST /api/v1/admin/me/step-down` | `user:update_role` (admin) | Give up your own admin role (`{new_role}`). Refused while you are the last active admin, so a handover is promote-then-stand-down. Audited. |
